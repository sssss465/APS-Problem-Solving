...
// THIS WILL TLE AS THE DP FUNCTIONALITY HAS BEEN REMOVED, LEAVING O(n^2m^2).

int grid[MAXN][MAXM]; // Where the grid is stored
int n, m;             // size of grid

int dists[MAXN][MAXM];     // Here we store the length of the longest path to that gridistsoint
...
int solve(int row, int col)
{
    ...
    dists[row][col] = 1; // If it's not outside the grid, then at the very least there is a distance of 1 there.
    ...
    for (int i = 0; i < 8; i += 2)
    {
        if (grid[dirs[i]][dirs[i + 1]] < grid[row][col]) // if the value is downhill we recurese.
        {
            ...
            if (s > dists[row][col])                 // we update the length and neighbor from which we came if s is greater
            {
                dists[row][col] = s;
                neighbor[row][col] = dirs_c[i / 2];
            }
            else if (s == dists[row][col]) // if s is the same, we make sure we take the neighbor with the lower lexographical value
            {
    ...
    if (dists[row][col] > maximum) // maximum tracks the longest length.
    {
        maximum = dists[row][col];
